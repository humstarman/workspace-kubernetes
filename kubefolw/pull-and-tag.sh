...
for GO_IMAGE in $GO_IMAGES; do
  NAME=${GO_IMAGE##*/}
  #echo $NAME
  docker pull ${DOCKER_HUB}/$NAME
  echo "$(date) - [INFO] - image ${DOCKER_HUB}/$NAME pulled."
  docker tag ${DOCKER_HUB}/$NAME ${LOCAL_REPO}/$NAME
  echo "$(date) - [INFO] - rename ${DOCKER_HUB}/$NAME as ${LOCAL_REPO}/$NAME."
